...
모든 서비스가 플랫폼은 아닙니다. 외부 데이터 소스를 감싼 수백 줄짜리, 내부 기능 하나, 고객사 하나의 소박한 백엔드도 있습니다. **그리고 이런 것들이 대충 만들어집니다.** 누가 신경 써서 설계하기엔 너무 작아서, 각자 자기 설정 방식·자기 배포 의식·물려받는 사람을 위한 자기만의 함정을 갖게 됩니다.

**어려웠던 지점**

- **셋업 비용은 고정이라 작은 서비스에서 지배적입니다.** Dockerfile, 매니페스트, 시크릿 처리, 헬스체크, 이미지 빌드·푸시 — 서비스가 300줄이든 30,000줄이든 같은 몇 시간입니다. 작은 서비스에서는 **그 오버헤드가 프로젝트의 대부분**입니다.
- **분기(divergence)는 즉시, 그리고 영구히 일어납니다.** 공유 템플릿 없이 한 달 간격으로 만든 두 서비스는 설정을 읽는 방식, 로그를 남기는 위치, 헬스 엔드포인트의 의미, 배포 방법이 전부 다릅니다. **아무도 나중에 돌아가서 맞추지 않습니다.**
- **배포 지식이 한 사람의 셸 히스토리에 삽니다.** 작은 서비스는 드물게 배포되므로 명령어 순서가 기록이 아니라 기억으로 남습니다. 그 기억을 가진 사람이 휴가 갈 때까지.
- **작다고 안 중요한 게 아닙니다.** 외부 데이터 소스 래퍼가 고객이 보고 있는 페이지 위에 있습니다. 아무도 일주일을 쓰지 않을 서비스여도 **헬스체크와 시크릿 처리는 다른 것과 똑같이** 필요합니다.

**접근**

- **모든 서비스가 쓰는 하나의 계층 구조** (`api/` 라우트, `services/` 비즈니스 로직, `clients/` 외부 시스템별 클라이언트, `models/` 스키마, `core/` 설정·DB·로깅). 이 구조가 최적이라서가 아니라 **어디서나 같기 때문에** 값어치가 있습니다. 처음 보는 서비스를 열어도 뭘 읽기 전에 어디를 볼지 압니다. 특히 `clients/`가 제 몫을 합니다 — 이 부류의 서비스는 전부 외부의 무언가와 대화하려고 존재하고, **거기에 고정된 거처를 주면 외부 의존성이 비즈니스 로직으로 번지지 않습니다.**
- **타입 있는 설정, 부팅 시 검증, 필수 값에 기본값 없음.** 누락된 필수 값은 **필드명과 함께 부팅에서** 실패합니다. 대안(진짜 자격증명 자리를 조용히 대신하는 기본값)은 훨씬 나중에, **다른 데의 버그처럼 보이는 방식으로** 실패합니다.
- **배포 인터페이스는 Make.** 빌드·푸시·배포. 타깃 3개, 모든 서비스에서 같은 3개. 아래의 명령은 달라도 **사람이 기억해야 할 인터페이스는 안 달라집니다.** 여기가 레버리지가 가장 큽니다 — 배포 지식이 누군가의 기억에서 **코드와 함께 사는 파일**로 옮겨갑니다.
- **시크릿은 커밋된 예시로, 절대 값으로는 안 됩니다.** 예시 파일이 서비스에 필요한 모든 변수를 값 없이 나열합니다. 실제 값은 클러스터가 주입합니다. **요구사항은 발견 가능하게 하면서 자격증명은 노출하지 않습니다** — 새로 온 사람에게 실제로 필요한 건 시크릿이 아니라 목록입니다.
- **의미 있는 헬스 엔드포인트.** 모든 서비스에 같은 경로로 존재하고 컨테이너 프로브에 연결됩니다. 클러스터 설정이 복붙이 될 만큼 균일한데, **아무도 생각하지 않아도 되는 것**에는 그게 정확히 원하는 바입니다.

**의도적으로 표준화하지 않은 것** — 비즈니스 로직, 데이터 모델, DB 유무. 템플릿은 서비스가 **어떻게 설정되고 빌드되고 배포되는지**, 즉 그릇에 대한 모든 것만 다룹니다. 안에서 무엇을 하는지는 달라야 하는 부분이고, **거기까지 손대는 템플릿은 쓰이는 대신 싸움의 대상**이 됩니다.
