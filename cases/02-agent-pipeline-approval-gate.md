...
**형태** — 4단계: 분석 → 실행 → 검토 → 승인
**요점** — 승인 단계가 곧 설계이고, 나머지는 배관
**규모** — 의도적으로 작은 내부 서비스

자연어로 들어온 요청을 받아 무엇을 해야 하는지 파악하고, 정해진 도구 집합으로 수행하고, 자기 출력을 점검한 다음, **확정 전에 멈춰서 사람에게 묻는** 서비스입니다.

**어려웠던 지점**

- **완전 자율은 틀렸고 완전 수동은 무의미합니다.** 검토 없이 행동하는 에이전트는 언젠가 아무도 원하지 않은 일을 저지릅니다. 매 단계를 묻는 에이전트는 손으로 하는 것보다 느립니다. 가치는 전부 그 경계를 어디에 두느냐에 달려 있습니다.
- **단일 프롬프트 에이전트는 자기를 검증하지 못합니다.** 한 번에 작업하고 자기 출력까지 검증하라고 하면 검토가 아니라 자기 평가가 나옵니다. 비평자는 지시가 다른 별도 단계여야 합니다. 아니면 자기 말에 동의하고 끝납니다.
- **피해는 도구에서 납니다.** 추론은 무해하지만 도구 호출은 그렇지 않습니다. 에이전트가 호출할 수 있는 것이 곧 저지를 수 있는 최악을 정합니다. 그러니 그 표면은 저절로 생겨나는 게 아니라 열거 가능해야 합니다.
- **멀티턴 작업에는 상태가 필요하고, 상태는 어딘가에 있어야 합니다.** 승인을 기다리며 멈춘 작업은 답이 돌아올 때까지 남아 있어야 합니다. 한참 뒤일 수도 있고, 아예 다른 요청에서 답이 올 수도 있습니다.

**접근**

- **책임이 나뉜 4단계.** 각 단계가 자체 프롬프트와 자체 책임을 가진 컴포넌트입니다. 분석과 실행을 나누면 도구가 돌기 전에 의도가 확정됩니다. 검토와 실행을 나누면 방금 그걸 만든 쪽이 검토를 맡지 않습니다. 검토와 자기합리화가 여기서 갈립니다.
- **도구는 발견되는 게 아니라 선언됩니다.** 무엇이 존재하는지 명시한 정의가 있고, 그것을 실행하는 executor가 따로 있습니다. 가능한 행동의 집합이 사람이 읽을 수 있는 목록이고, 시스템의 최악을 따져보려면 그 방법밖에 없습니다.
- **승인 게이트는 콜백이 아니라 상태입니다.** 승인을 기다리는 작업은 이후 메시지가 재개시키는 영속 상태로 남습니다. 그래서 파이프라인이 도는 동안 누가 앉아 있지 않아도 몇 분, 몇 시간 뒤에 별도 상호작용으로 승인할 수 있습니다.
- **단계 간 계약은 타입으로.** 각 단계의 입출력을 스키마로 선언합니다. 네 단계가 구조화된 상태를 주고받는데 타입 없는 딕셔너리를 쓰면, 한 단계를 고친 것이 다른 단계에서 조용한 고장으로 나타납니다.

**솔직한 한계** — 작업 상태를 메모리에 들고 있습니다. 동시 작업이 몇 개 안 되는 내부 서비스에는 충분하고, 여기에 뭔가를 실제로 의존시키기 전에 공유 저장소로 바꿀 것이라고 코드에 표시해뒀습니다. 서비스를 재시작하면 진행 중이던 승인은 사라집니다. 실수가 아니라 범위를 정한 결과입니다 — 파이프라인 형태를 잡으려고 만든 서비스이고, 영속화는 알고 있는 다음 단계입니다. 그래도 명시할 값어치는 있습니다. 실패가 조용하기 때문입니다. 테스트에서는 다 되고, 하필 승인 도중에 배포가 나가는 프로덕션에서 작업을 잃습니다.

**일반화되는 것**

- **행위자와 비평자를 분리한다.** 자기 점검이 필요한 시스템이라면 점검은 지시가 다른 별도 단계가 맡아야 합니다. 에이전트에만 해당하는 이야기가 아닙니다.
- **행동 표면을 열거한다.** 시스템의 폭발 반경은 그것이 일으킬 수 있는 부작용의 집합입니다. 그 집합이 선언되어 있지 않고 저절로 생겨난 것이라면 아무도 크기를 가늠할 수 없습니다.
- **승인은 인터럽트가 아니라 상태 기계다.** 사람의 결정을 블로킹 호출로 다루면 그 사람이 대기 중이라고 가정하는 셈입니다. 무언가가 나중에 재개시키는 영속 상태로 다루면 대기 중이 아니라고 가정하는 셈이고, 현실은 그쪽입니다.
